...
import (
	...
	"github.com/ecosystem/go-ecosystem/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gehc console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = ehc.getBlock(ehc.getTransaction(tx).blockHash);
  var genesis = ehc.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.ehc.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  ehc.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(ehcdb.NewMemDatabase(), test.Genesis.Alloc)
