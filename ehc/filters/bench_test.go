...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/gehc/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := ehcdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = ehcdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db ehcdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*ehcdb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db ehcdb.Database) {
