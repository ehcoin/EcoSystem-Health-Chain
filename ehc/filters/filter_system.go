...
// Package filters implements an ecosystem filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  ecosystem.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit ecosystem.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit ecosystem.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit ecosystem.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit ecosystem.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
