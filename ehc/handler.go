...
import (
	...
	"github.com/ecosystem/go-ecosystem/ehc/fetcher"
...
// NewProtocolManager returns a new Ecosystem sub protocol manager. The Ecosystem sub protocol manages peers capable
// with the Ecosystem network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ehcdb.Database, MsgCenter *mc.Center) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		if mode == downloader.FastSync && version < ehc63 {
			continue
		}
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Ecosystem peer", "peer", id)

	// Unregister the peer from the downloader and Ecosystem peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Ecosystem protocol")
	...
	log.Info("Ecosystem protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an ehc peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Ecosystem peer connected", "name", p.Name())

	// Execute the Ecosystem handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Ecosystem handshake failed", "err", err)
		return err
	}
	...
	if err := pm.Peers.Register(p); err != nil {
		p.Log().Error("Ecosystem peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Ecosystem message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= ehc63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= ehc63 && msg.Code == NodeDataMsg:
	...
	case p.version >= ehc63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= ehc63 && msg.Code == ReceiptsMsg:
...
// NodeInfo represents a short summary of the Ecosystem sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Ecosystem network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
