...
// PeerInfo represents a short summary of the Ecosystem sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Ecosystem protocol version negotiated
...
// Handshake executes the ehc protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("ehc/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Ecosystem sub-protocol.
type peerSet struct {
