...
// PublicEcosystemAPI provides an API to access Ecosystem related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicEcosystemAPI struct {
	b Backend
}

// NewPublicEcosystemAPI creates a new Ecosystem protocol API.
func NewPublicEcosystemAPI(b Backend) *PublicEcosystemAPI {
	return &PublicEcosystemAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicEcosystemAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Ecosystem protocol version this node supports
func (s *PublicEcosystemAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicEcosystemAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Ecosystem Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Ecosystem Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Ecosystem ECDSA signature for:
// keccack256("\x19Ecosystem Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/ecosystem/go-ecosystem/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Ecosystem Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/ecosystem/go-ecosystem/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Ecosystem signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Ecosystem blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Ecosystem blockchain API.
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	if args.Gas == nil {
		...
		if len(args.ExtraTo) > 0 && args.LockHeight > 0 && args.TxType > 0 {
			*(*uint64)(args.Gas) = 90000 * uint64(len(args.ExtraTo))
		} else {
...
//
// https://github.com/ecosystem/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Ecosystem APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", ehcash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Ecosystem APIs exposed over the private
// debugging endpoint.
...
// Version returns the current ecosystem protocol version.
