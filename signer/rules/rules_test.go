...
import (
	...
	"github.com/ecosystem/go-ecosystem/signer/core"
	"github.com/ecosystem/go-ecosystem/signer/storage"
)
...
func (alwaysDenyUI) OnApprovedTx(tx ehcapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx ehcapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(ehcapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := ehcapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx ehcapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
