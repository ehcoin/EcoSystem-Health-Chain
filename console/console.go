...
import (
	...
	"github.com/ecosystem/go-ecosystem/internal/jsre"
	"github.com/ecosystem/go-ecosystem/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Ecosystem requests through
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var ehc = web3.ehc; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = ehc.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ehc.getBalance(ehc.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gehc instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gehc metadata
	fmt.Fprintf(c.printer, "Welcome to the Gehc JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + ehc.coinbase);
		console.log("at block: " + ehc.blockNumber + " (" + new Date(1000 * ehc.getBlock(ehc.blockNumber).timestamp) + ")");
