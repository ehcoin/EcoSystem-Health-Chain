...
import (
	...
	"github.com/ecosystem/go-ecosystem/les/flowcontrol"
...
func NewLesServer(ehc *ehc.Ecosystem, config *ehc.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(ehc.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, ehc.EventMux(), ehc.Engine(), newPeerSet(), ehc.BlockChain(), ehc.TxPool(), ehc.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(ehc.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(ehc.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(ehc.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(ehc.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(ehc.ChainDb())
	return srv, nil
}
...
func newCostStats(db ehcdb.Database) *requestCostStats {
