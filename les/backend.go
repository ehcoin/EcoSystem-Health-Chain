...
import (
	...
	rpc "github.com/ecosystem/go-ecosystem/rpc"
)

type LightEcosystem struct {
	config *ehc.Config
...
func New(ctx *node.ServiceContext, config *ehc.Config) (*LightEcosystem, error) {
	chainDb, err := ehc.CreateDB(ctx, config, "lightchaindata")
	if err != nil {
		return nil, err
	}
	...
	leth := &LightEcosystem{
		...
		engine:           ehc.CreateConsensusEngine(ctx, &config.Ethash, chainConfig, chainDb),
		shutdownChan:     make(chan bool),
		networkId:        config.NetworkId,
		bloomRequests:    make(chan chan *bloombits.Retrieval),
		bloomIndexer:     ehc.NewBloomIndexer(chainDb, light.BloomTrieFrequency),
...
// APIs returns the collection of RPC services the ecosystem package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightEcosystem) APIs() []rpc.API {
	return append(ehcapi.GetAPIs(s.ApiBackend), []rpc.API{
		{
			Namespace: "eth",
			Version:   "1.0",
...
func (s *LightEcosystem) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightEcosystem) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightEcosystem) TxPool() *light.TxPool              { return s.txPool }
func (s *LightEcosystem) Engine() consensus.Engine           { return s.engine }
func (s *LightEcosystem) LesVersion() int                    { return int(s.protocolManager.SubProtocols[0].Version) }
func (s *LightEcosystem) Downloader() *downloader.Downloader { return s.protocolManager.downloader }
func (s *LightEcosystem) EventMux() *event.TypeMux           { return s.eventMux }

// Protocols implements node.Service, returning all the currently configured
// network protocols to start.
func (s *LightEcosystem) Protocols() []p2p.Protocol {
	return s.protocolManager.SubProtocols
}

// Start implements node.Service, starting all internal goroutines needed by the
// Ecosystem protocol implementation.
func (s *LightEcosystem) Start(srvr *p2p.Server) error {
	s.startBloomHandlers()
	log.Warn("Light client mode is an experimental feature")
	s.netRPCService = ehcapi.NewPublicNetAPI(srvr, s.networkId)
...
// Ecosystem protocol.
func (s *LightEcosystem) Stop() error {
	s.odr.Stop()
