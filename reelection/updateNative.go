...
func (self *ReElection) ToNativeMinerStateUpdate(height uint64, allNative AllNative) (AllNative, error) {
	...
	if header==nil{
		log.ERROR(Module,"处理矿工初选列表更新-获取指定高度的区块头失败 高度",height)
		return allNative,errors.New("获取指定高度的区块头失败")
	}
	...
	log.INFO(Module, "ToNativeMinerStateUpdate", "end","高度",height)
...
func (self *ReElection) ToNativeValidatorStateUpdate(height uint64, allNative AllNative) (AllNative, error) {
	...
	header:=self.bc.GetHeaderByNumber(height)
	if header==nil{
		log.ERROR(Module,"处理验证者初选列表更新-获取指定高度的区块头失败 高度",height)
		return allNative,errors.New("获取指定高度的区块头失败")
	}
	DiffFromBlock := header.NetTopology
	...
	log.INFO(Module, "ToNativeValidatorStateUpdate", "end","高度",height)
	return allNative, nil
}
