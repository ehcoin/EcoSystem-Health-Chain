...
func InitLog(verbosity uint32) {
	Root().SetHandler(NewTestHandler(verbosity))
}
...
func NewTestHandler(verbosity uint32) *TestHandler {
	return &TestHandler{
		fmtr:  TerminalFormat(false),
		level: verbosity,
		//		fmtr : JSONFormatEx(false,true),
		//		fmtr : JSONFormatEx(false,true),
	}
}
...
func (h *TestHandler) Verbosity(level Lvl) {
...
func (h *TestHandler) Log(r *Record) error {
	...
	if atomic.LoadUint32(&h.level) >= uint32(r.Lvl) {
		fmt.Print(r.Call.String(), ": \t", string(h.fmtr.Format(r)))
		return nil
	}
