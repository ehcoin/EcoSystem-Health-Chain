...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (ehcdb.Database, error) {
	if ctx.config.DataDir == "" {
		return ehcdb.NewMemDatabase(), nil
	}
	db, err := ehcdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
