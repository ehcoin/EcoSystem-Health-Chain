...
func (n *Node) OpenDatabase(name string, cache, handles int) (ehcdb.Database, error) {
	if n.config.DataDir == "" {
		return ehcdb.NewMemDatabase(), nil
	}
	return ehcdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
