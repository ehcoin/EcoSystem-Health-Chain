...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("ehc", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("ehc", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-ecosystem-test-ipc-%d-%d", os.Getpid(), rand.Int63())
