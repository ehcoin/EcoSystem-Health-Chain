...
func TestRightRequestMsg_01(t *testing.T) {
	ehc := newEth(t, nil, false)

	reElection, err := reelection.New(ehc.BlockChain(), nil)
	if err != nil {
		t.Fatalf("create reelection server err!")
	}

	blkVerify, err := NewBlockVerify(ehc)
	if err != nil {
		t.Fatalf("StartServer failed: %v\n", err)
	}

	number := ehc.BlockChain().CurrentHeader().Number.Uint64()
	blkVerify.handleRoleUpdatedMsg(&mc.RoleUpdatedMsg{Role: common.RoleValidator, BlockNum: number, Leader: common.Address{}})
	leaderMsg := &mc.LeaderChangeNotify{
		ConsensusState: true,
		Leader:         common.HexToAddress("FDABEFF1234"),
	...
	requstMsg := &mc.BlockVerifyConsensusReq{
		Header:  createHeader(t, ehc.BlockChain(), leaderMsg.Leader),
		TxsCode: nil,
	}
...
func newEth(t *testing.T, confOverride func(*ehc.Config), isBroadcastNode bool) *ehc.Ecosystem {
	...
	// Create a networkless protocol stack and start an Ecosystem service within
	...
	ehcConf := &ehc.Config{
		...
		Ethash: ehcash.Config{
			PowMode: ehcash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(ehcConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return ehc.New(ctx, ehcConf) }); err != nil {
		t.Fatalf("failed to register Ecosystem protocol: %v", err)
	}
