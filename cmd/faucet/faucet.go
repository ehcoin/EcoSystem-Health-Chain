...
var (
	...
	ehcPortFlag = flag.Int("ehcport", 10909, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Ecosystem protocol")
	statsFlag   = flag.String("ehcstats", "", "Ethstats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Ecosystem and the faucet")
)

var (
	ehc = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	faucet, err := newFaucet(genesis, *ehcPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Ecosystem address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Ecosystem light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Ecosystem protocol stack
	client *ehcclient.Client   // Client connection to the Ecosystem chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Ecosystem light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := ehc.DefaultConfig
	...
	// Assemble the ehcstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightEcosystem
			ctx.Service(&serv)
			return ehcstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := ehcclient.NewClient(api)
...
// close terminates the Ecosystem connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Ecosystem address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/ecosystem/go-ecosystem/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), ehc)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, ehc)
...
// the username, avatar URL and Ecosystem address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Ecosystem addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Ecosystem address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Ecosystem address and profile URL.
...
// returning the username, avatar URL and Ecosystem address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Ecosystem address,
