...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"ehcstats", "explorer", "wallet", "faucet"} {
	...
	// If we have ehcstats running, ask whether to make the secret public or not
	if w.conf.ehcstats != "" {
		fmt.Println()
		fmt.Println("Include ehcstats secret on dashboard (y/n)? (default = yes)")
