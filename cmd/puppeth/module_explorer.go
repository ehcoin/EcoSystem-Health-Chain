...
ADD ehcstats.json /ehcstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../ehc-net-intelligence-api && pm2 start /ehcstats.json)' >  explorer.sh && \
	echo '(cd ../ehcchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerEthstats is the configuration file for the ehcstats javascript client.
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.ecosystem
    environment:
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	ehcstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEthstats)).Execute(ehcstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.ehcstats[:strings.Index(config.ehcstats, ":")],
		"Secret": config.ehcstats[strings.Index(config.ehcstats, ":")+1 : strings.Index(config.ehcstats, "@")],
		"Host":   config.ehcstats[strings.Index(config.ehcstats, "@")+1:],
	})
	files[filepath.Join(workdir, "ehcstats.json")] = ehcstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.ecosystem"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		ehcstats: infos.envvars["STATS"],
	}
	return stats, nil
}
