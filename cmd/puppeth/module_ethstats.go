...
// ehcstatsDockerfile is the Dockerfile required to build an ehcstats backend
// and associated monitoring site.
var ehcstatsDockerfile = `
FROM puppeth/ehcstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// ehcstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an ehcstats monitoring site.
var ehcstatsComposefile = `
version: '2'
services:
  ehcstats:
    build: .
    image: {{.Network}}/ehcstats{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new ehcstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(ehcstatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(ehcstatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the ehcstats service
	if nocache {
...
// ehcstatsInfos is returned from an ehcstats status check to allow reporting
// various configuration parameters.
type ehcstatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *ehcstatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an ehcstats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*ehcstatsInfos, error) {
	// Inspect a possible ehcstats container on the host
