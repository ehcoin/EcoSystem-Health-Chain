...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for ehcstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["ehcstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["ehcstats"] = infos.Report()
		ehcstats = infos.config
	}
