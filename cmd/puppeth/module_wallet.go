...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gehc --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gehc --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ehcstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppethNetworkID/{{.NetworkID}}/g' dist/js/ehcwallet-master.js && \
	sed -i 's/PuppethNetwork/{{.Network}}/g'     dist/js/ehcwallet-master.js && \
	sed -i 's/PuppethDenom/{{.Denom}}/g'         dist/js/ehcwallet-master.js && \
	sed -i 's/PuppethHost/{{.Host}}/g'           dist/js/ehcwallet-master.js && \
	sed -i 's/PuppethRPCPort/{{.RPCPort}}/g'     dist/js/ehcwallet-master.js
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.RPCPort}}:9099"{{if not .VHost}}
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.ecosystem
    environment:
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["9099/tcp"]
