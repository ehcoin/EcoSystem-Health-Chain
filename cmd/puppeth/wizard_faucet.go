...
func (w *wizard) deployFaucet() {
	...
	if err != nil {
		infos = &faucetInfos{
			node:    &nodeInfos{port: 10909, peersTotal: 25},
			port:    80,
			host:    client.server,
			amount:  1,
			minutes: 1440,
	...
	fmt.Println()
	if infos.node.ehcstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.ehcstats = w.readString() + ":" + w.conf.ehcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.ehcstats)
		infos.node.ehcstats = w.readDefaultString(infos.node.ehcstats) + ":" + w.conf.ehcstats
	}
