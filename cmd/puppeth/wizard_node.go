...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.ehcstats == "" {
		log.Error("No ehcstats server configured")
		return
	}
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 10909, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 10909, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	existed := err == nil
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.ehcashdir == "" {
			fmt.Printf("Where should the ehcash mining DAGs be stored on the remote machine?\n")
			infos.ehcashdir = w.readString()
		} else {
			fmt.Printf("Where should the ehcash mining DAGs be stored on the remote machine? (default = %s)\n", infos.ehcashdir)
			infos.ehcashdir = w.readDefaultString(infos.ehcashdir)
		}
	}
	...
	if infos.ehcstats == "" {
		...
		infos.ehcstats = w.readString() + ":" + w.conf.ehcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.ehcstats)
		infos.ehcstats = w.readDefaultString(infos.ehcstats) + ":" + w.conf.ehcstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an ehcbase to mine against
			fmt.Println()
			if infos.ehcbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.ehcbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.ehcbase)
				infos.ehcbase = w.readDefaultAddress(common.HexToAddress(infos.ehcbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Ecosystem node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
