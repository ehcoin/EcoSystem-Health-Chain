...
type config struct {
	...
	ehcstats  string   // Ethstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // Ecosystem services known to be running on servers
...
// converts it to an Ecosystem address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by ehcstats, which compares IPs textually, not by value.
