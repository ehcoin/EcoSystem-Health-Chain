...
func TestAccountListEmpty(t *testing.T) {
	gehc := runGehc(t, "account", "list")
	gehc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gehc := runGehc(t, "account", "list", "--datadir", datadir)
	defer gehc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gehc := runGehc(t, "account", "new", "--lightkdf")
	defer gehc.ExpectExit()
	gehc.Expect(`
...
`)
	gehc.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gehc := runGehc(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gehc.ExpectExit()
	gehc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gehc.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gehc.StderrText(), m) {
...
// https://github.com/ecosystem/go-ecosystem/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gehc.SetTemplateFunc("keypath", func(file string) string {
