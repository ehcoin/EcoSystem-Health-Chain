...
// Copyright 2014 The go-ecosystem Authors
...
// gehc is the official command-line client for Ecosystem.
package main
...
import (
	...
	"github.com/ecosystem/go-ecosystem/console"
...
const (
	clientIdentifier = "gehc" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the go-ecosystem command line interface")
...
func init() {
	// Initialize the CLI app and start Gehc
	app.Action = gehc
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2018 The go-ecosystem Authors"
...
// gehc is the main entry point into the system if no sprguments and runs it in
...
func gehc(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	go func() {
		...
		stateReader := ehcclient.NewClient(rpcClient)
	...
	var ecosystem *ehc.Ecosystem
	if err := stack.Service(&ecosystem); err != nil {
		utils.Fatalf("Ecosystem service not running :%v", err)
	}
	...
	log.INFO("创世文件选举信息", "data", ecosystem.BlockChain().GetBlockByNumber(0).Header().Elect)
	log.INFO("创世文件拓扑图", "data", ecosystem.BlockChain().GetBlockByNumber(0).Header().NetTopology)

	// Start auxiliary services if enabled
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Ecosystem node is running
		...
		if threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok := ecosystem.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}
		// Set the gas price to the limits from the CLI and start mining
		ecosystem.TxPool().SetGasPrice(utils.GlobalBig(ctx, utils.GasPriceFlag.Name))
		if err := ecosystem.StartMining(true); err != nil {
