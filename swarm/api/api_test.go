...
import (
	...
	"github.com/ecosystem/go-ecosystem/swarm/storage"
)
...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.ehc"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.ehc"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.ehc"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	ehcAddr := "swarm.ehc"
	ehcHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	ehcResolve := newTestResolver(ehcHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(ehcResolve, "")),
			addr:   ehcAddr,
			result: ehcHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(ehcResolve, ""),
				MultiResolverOptionWithResolver(ehcResolve, ""),
			),
			addr:   ehcAddr,
			result: ehcHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(ehcResolve, "ehc"),
			),
			addr:   ehcAddr,
			result: ehcHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "ehc"),
				MultiResolverOptionWithResolver(doesntResolve, "ehc"),
				MultiResolverOptionWithResolver(ehcResolve, "ehc"),
			),
			addr:   ehcAddr,
		...
		},
		{
			...
			addr:   ehcAddr,
			result: ehcHash,
			err:    errors.New(`DNS name not found: "swarm.ehc"`),
		},
		{
