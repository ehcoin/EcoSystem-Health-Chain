...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Ecosystem
...
type wallet struct {
	...
	deriveChain    ecosystem.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Ecosystem account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain ecosystem.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
func (w *wallet) SignHashValidate(account accounts.Account, hash []byte, validate bool) ([]byte, error) {
	return nil, accounts.ErrNotSupported
}
