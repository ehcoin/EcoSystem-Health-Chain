...
import (
	...
	"github.com/ecosystem/go-ecosystem/accounts/usbwallet/internal/trezor"
...
// or whether the Ecosystem app was not started on it.
...
// Ecosystem address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.EcosystemAddress)
	if _, err := w.trezorExchange(&trezor.EcosystemGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.EcosystemSignTx{
	...
	response := new(trezor.EcosystemTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EcosystemTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
